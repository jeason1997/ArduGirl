...
项目当前实现 Linux 纯终端前端。默认示例是 Arduboy2 官方 `HelloWorld.ino`，上游文件通过 Git 子模块引入并保持不修改。SDL2 和 MCU 后端将在终端兼容基线稳定后实现。

## 快速开始

环境要求：Linux 或 WSL、GNU Make、支持 C++17 的 GCC/Clang、UTF-8 终端。
