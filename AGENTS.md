...
- 根目录 `Makefile` 只能描述核心、平台、公共测试和游戏构建扩展点。不得出现具体游戏名、游戏集合名、上游私有类型、游戏专用源码路径、补丁名或条件白名单。游戏和集合必须通过 `games/examples/*/port.mk` 或 `games/ports/*/port.mk` 自行声明构建、上游检查、生成步骤和测试；根 Makefile 只能通配加载这些模块并聚合通用目标。
