...
- 非正式、临时或为定位问题随机增加的测试输出、日志、转储和中间文件，禁止写入工程目录。
- 需要保留诊断输出时必须使用操作系统缓存或临时目录；验证结束后应主动清理。只有经过设计、属于长期回归体系的正式测试源码和固定基准，才允许按仓库架构放入对应测试目录。
- 2026-08-07 曾把 `py32-ardynia.log` 等临时构建日志直接写入仓库根目录，污染用户工作区。此错误不得重复；运行命令前必须先检查所有重定向目标是否位于工程外的缓存目录。
...
- 根目录 `Makefile` 只能作为平台选择和目标转发入口，不得声明编译器参数、平台源码、公共测试对象或游戏构建细节。不得出现具体游戏名、游戏集合名、上游私有类型、游戏专用源码路径、补丁名或条件白名单。平台构建规则必须放在 `platform/<platform>/Makefile`；每款游戏以 `game.toml` 作为唯一必需的构建描述，普通游戏不得新增 `port.mk`、手写运行入口或平台专用构建文件。
- 使用 `GAME=<game-id>` 选择单个游戏时，平台构建必须先按 `game.toml` 定位并只展开该游戏的规则和依赖。禁止先展开全部游戏或加载全部 `.d` 文件，再从最终目标列表中过滤指定游戏；该反模式会在 WSL 的 `/mnt/*` DrvFS 工作区造成数秒级解析延迟。只有未指定 `GAME` 的全量构建、聚合测试或明确的多游戏任务才允许加载全部游戏。
- 同仓库多游戏集合必须从每个游戏的 `game.toml` 自动发现目标，不得维护游戏名列表。只有真实的集合共有源码布局差异才允许由各游戏清单显式引用集合级 `profile.toml`；profile 不得包含平台、工具链或最终目标规则，也不得由构建器按父目录隐式猜测。新增规范游戏后不应修改根 Makefile 或平台 Makefile。
...
- 修改上游源码只能使用游戏 `game.toml` 显式引用的 `patches`；集合 profile 和构建清单不得另设文本替换、正则替换或可选静默替换机制。
...
- 用户要求“全部修复”“全部测试”或“不许遗漏”时，必须先枚举完整目标集合，并以该集合中每个目标的独立结果作为验收清单。
- 全量构建必须等待聚合命令完整结束并检查最终退出状态；超时、输出截断、代表样本通过或部分结果都不得表述为完成。
- 只要清单中仍有一个失败、未运行或无法确认的目标，就必须明确报告未完成及剩余项，不得使用“已完成修复”“全部通过”等措辞。
- 修复过程中新增的生成规则也必须纳入同一轮空产物验证，不能把新规则只对一个样本有效误报为集合级修复。
- 本约束记录 2026-08-07 PY32 批量修复中的实际错误：Agent 在 24 款游戏仅验证少数样本、全量命令尚未完成且 18 款仍失败时提前声称完成。后续不得再次以抽样结果替代用户明确要求的全量结果。
