...
- 所有源代码注释必须使用中文，包括普通注释、文档注释、TODO、FIXME 和测试说明；不得出现英文注释。
- 代码必须提供详细注释，重点解释接口契约、位布局、边界条件、平台差异、时间回绕、所有权和非直观优化；不能只把代码逐句翻译成中文。
...
- 原始源码尽量放在 `upstream/`，ArduGirl 适配放在 `port/` 或补丁文件中。
