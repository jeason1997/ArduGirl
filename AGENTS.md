...
- 根目录 `Makefile` 只能作为平台选择和目标转发入口，不得声明编译器参数、平台源码、公共测试对象或游戏构建细节，也不得加载 `games/*/port.mk`。不得出现具体游戏名、游戏集合名、上游私有类型、游戏专用源码路径、补丁名或条件白名单。平台构建规则必须放在 `platform/<platform>/Makefile`，游戏和集合必须通过自身 `games/*/port.mk` 声明构建、上游检查、生成步骤和测试。
- 使用 `GAME=<game-id>` 选择单个游戏时，平台 Makefile 必须先根据直属游戏目录或集合内 `game.toml` 定位所属 `port.mk`，然后只加载该模块和该游戏的依赖。禁止先通配加载全部游戏模块或全部 `.d` 文件，再从最终目标列表中过滤指定游戏；该反模式会在 WSL 的 `/mnt/*` DrvFS 工作区造成数秒级解析延迟。只有未指定 `GAME` 的全量构建、聚合测试或明确的多游戏任务才允许加载全部移植模块。
